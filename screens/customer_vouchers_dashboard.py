...
from screens.customer_vouchers_form import CustomerVouchersForm

class CustomerVouchersDashboardFrame(BaseView):
    def __init__(self, master):
        super().__init__(master, "Customer Vouchers", add_button_command=self.goto_add)
    ...
    def build_body(self):
        ...
        self.tree = ttk.Treeview(
            table_frame,
            columns=("id", "name", "quantity", "price", "date", "user"),
            show="headings",
        ...
        for col in ("id", "name", "quantity", "price", "date", "user"):
    ...
    def refresh(self):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT id, name, quantity, price, date, user FROM customer_vouchers")
        ...
        columns = [
            ("id", "ID"), ("name", "Name"), ("quantity", "Quantity (L)"),
    ...
    def goto_add(self):
        ...
        window.title("Add Customer Voucher")
        ...
        form = CustomerVouchersForm(window, self.master, self)
