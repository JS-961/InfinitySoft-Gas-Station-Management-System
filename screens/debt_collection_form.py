...
class DebtCollectionForm(BaseForm):
    def __init__(self, window, app, dashboard):
        super().__init__(window, "Add Debt Collection Entry")
        self.master = app
        self.window = window
    ...
    def submit(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "INSERT INTO debt_collection (name, amount, date, user) VALUES (?, ?, ?, ?)",
        ...
        self.master.show_frame("DebtCollectionDashboardFrame")
