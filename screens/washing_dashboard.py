...
from screens.washing_form import WashingForm

class WashingDashboardFrame(BaseView):
    def __init__(self, master):
        super().__init__(master, "Washing", add_button_command=self.goto_add)
    ...
    def build_body(self):
        ...
        self.tree = ttk.Treeview(
            table_frame,
            columns=("id", "type", "price", "date", "user"),
            show="headings",
        ...
        for col in ("id", "type", "price", "date", "user"):
    ...
    def refresh(self):
        print("[WashingDashboardFrame] refresh called")
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT id, type, price, date, user FROM washing")
        rows = cursor.fetchall()
        conn.close()

        for i, row in enumerate(rows):
            tag = "evenrow" if i % 2 == 0 else "oddrow"
            display_price = self.master.convert_amount(row[2])
            self.tree.insert("", "end", values=(row[0], row[1], display_price, row[3], row[4]), tags=(tag,))
    ...
    def goto_add(self):
        ...
        window.title("Add Washing Entry")
        ...
        form = WashingForm(window, self.master, self)
